...
class ClassroomParticipationView extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ClassroomParticipationView> createState() =>
      _ClassroomParticipationViewState();
}

class _ClassroomParticipationViewState
    extends ConsumerState<ClassroomParticipationView> {
  ...
  Future<void> _initDefaultClassAndSession() async {
    ...
    try {
      ...
      if (targetClassId == null) {
        final Map<String, dynamic>? activeLesson = await ref.read(
          activeTimetableLessonProvider.future,
        );
      ...
      debugPrint(
        'ClassroomParticipationView._initDefaultClassAndSession Error: $e\n$stackTrace',
      );
    }
  }
  ...
  Future<void> _loadCurrentSession(
    ...
    try {
      ...
      debugPrint(
        'ClassroomParticipationView._loadCurrentSession Error: $e\n$stackTrace',
      );
    }
  }

  /// Kaydedilmemiş değerlendirme varken çıkış onayı ister.
  Future<bool?> _confirmDiscard(BuildContext context) {
    return showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Bu derste yaptığınız değerlendirmeler henüz kaydedilmedi. '
          'Çıkarsanız tümü kaybolur.',
        ),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    final selectedLessonHour = ref.watch(
      selectedParticipationLessonHourProvider,
    );
    ...
    final teacherName = teacherProfile.fullName.isNotEmpty
        ? teacherProfile.fullName
        : 'Ders Öğretmeni';
    final schoolName = teacherProfile.schoolName;

    // Kaydedilmemiş değerlendirme varken çıkışı engelle.
    //
    // Değerlendirme yalnızca "Kaydet" düğmesiyle veritabanına yazılıyor.
    // Bu koruma olmadan öğretmen 30 öğrenciyi değerlendirip geri tuşuna
    // basınca hepsi SESSİZCE kayboluyordu.
    return PopScope(
      canPop: !ref
          .read(currentParticipationSessionProvider.notifier)
          .hasUnsavedChanges,
      ...
      child: Scaffold(
        ...
        appBar: CustomAppBar(
          ...
          actions: [
            // Dönem/yıl sonu kümülatif raporlar.
            //
            ...
            IconButton(
              ...
              tooltip: 'Dönem & yıl sonu raporları',
              onPressed: () =>
            ...
            children: [
              ...
              Expanded(
                child: sessionAsync.when(
                  ...
                  error: (err, stack) {
                    debugPrint(
                      'ClassroomParticipationView Error: $err\n$stack',
                    );
                    return Center(
                      child: Padding(
                  ...
                  data: (sessionData) {
                    if (sessionData == null) {
                      return const Center(
                        child: Text('Lütfen bir sınıf seçiniz.'),
                      );
                    }
                    ...
                    return Column(
                      children: [
                        // Kullanim ipucu.
                        //
                        // Dokunmanin artik ne yaptigini soylemek SART:
                        // eskiden dokunmak diyalog aciyordu, ogretmen
                        // aliskanligi degisiyor.
                        Container(
                          width: double.infinity,
                          ...
                          child: Text(
                            'Dokun → söz hakkı ver  ·  Çift dokun → geri al'
                            '  ·  Uzun bas → detay',
                            textAlign: TextAlign.center,
                            style: AppFonts.outfit(
                              fontSize: 11,
                        ...
                        Expanded(
                          child: CompactStudentParticipationGrid(
                            session: sessionData,
                          ...
                          child: DropdownButton<int>(
                            ...
                            onChanged: (val) {
                              if (val != null) {
                                ref
                                        .read(
                                          selectedParticipationClassIdProvider
                                              .notifier,
                                        )
                                        .state =
                                    val;
              ...
              Expanded(
                ...
                child: InkWell(
                  onTap: () async {
                    ...
                    if (picked != null) {
                      final newDateStr = picked.toIso8601String().substring(
                        0,
                        10,
                      );
                      ref
                              .read(selectedParticipationDateProvider.notifier)
                              .state =
                          newDateStr;
                      ...
                      Expanded(
                        child: TextField(
                          ...
                          decoration: InputDecoration(
                            hintText:
                                'Öğrenci adı veya okul no ile filtrele...',
            ...
            child: ElevatedButton.icon(
              onPressed: () async {
                final success = await ref
                    ...
                    .saveCurrentSession();
                if (context.mounted) {
                  // Ders sonu ozeti: kac ogrenciyle konusuldu, kac kisi
                  // hic soz almadi. Ogretmenin "kimi atladim" sorusunun
                  // cevabi; eskiden bu bilgi hicbir yerde yoktu.
                  final konusan =
                      session.evaluations.where((e) => !e.isSilent).length;
                  final sessiz = session.evaluations.length - konusan;
